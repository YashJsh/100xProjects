...
export function AgentConversationPage() {
  ...
  // 1. Fetch conversation details & connect WebSocket
  useEffect(() => {
    if (id) {
  ...
  const handleClose = async () => {
    if (!id || activeConversation?.status === "CLOSED") return;
    try {
      closeConversationSocket(id);
        ...
        <div className="space-y-4 overflow-y-auto mb-4 flex-1">
          {isLoading && messages.length === 0 ? (
            ...
            messages.map((m) => {
              ...
              const isMe = m.senderId === user?.id || m.senderRole === "AGENT";
